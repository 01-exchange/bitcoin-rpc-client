...
public class BitcoinJSONRPCClientTest {
    ...
    public void transactionDetailsTest()
    {
        ...
        ArrayList<HashMap<String, Object>> details = transaction.details();

        assertEquals("2NBFSxR84pvjGbw4LAE2mxzPEniCbHHJVEj", details.get(0).get("address"));
        assertEquals( "send", details.get(0).get("category"));
        assertEquals(new BigDecimal("-1.00000000"), details.get(0).get("amount"));
        assertEquals(1L, details.get(0).get("vout"));
        assertEquals(new BigDecimal("-0.00003320"), details.get(0).get("fee"));
        assertEquals(false, details.get(0).get("abandoned"));
    }
}